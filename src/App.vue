...
<template>
  <div>
    <div class="w-full bg-slate-800 text-white">
      <div class="max-w-7xl mx-auto flex items-center gap-3 p-6 sm:px-8">
        <img alt="Logo" :src="logoUrl" class="w-8 h-8 inline-block" />
        <h1 class="flex-1 text-xl sm:text-2xl font-titillium font-semibold">
          ServerStatus
        </h1>
      </div>
    </div>
    <div class="max-w-7xl mx-auto p-6 sm:px-8">
      ...
      <div
        v-else-if="stats"
        class="flex flex-col items-stretch gap-6 mb-6"
      >
        <div
      ...
      <p v-if="stats?.updated" class="mb-2 text-sm text-gray-500 dark:text-gray-400">
        Last updated: {{ new Date(stats?.updated * 1000).toLocaleString() }}
      </p>
      <p v-if="error" class="mb-2 text-sm text-red-600" v-text="error" />
      <p class="mb-2 text-xs text-gray-500 dark:text-gray-400">
        ...
        <a class="inline-block underline underline-offset-2" href="https://github.com/JingBh/ServerStatus-theme" target="_blank" rel="noreferrer">Theme</a>
      </p>
    </div>
  </div>
</template>
